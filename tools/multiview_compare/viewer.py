...
function setRenderOptions(){const sel=document.getElementById('renderSelect'),labels={full_gs:'预测位姿 · 全量GS',filtered_gs:'预测位姿 · 筛选GS',native:'预测位姿 · 原生GS',gt_aligned:'Sim(3) 对齐真实位姿'};sel.replaceChildren();(data.render_options||[]).forEach(k=>{const o=document.createElement('option');o.value=k;o.textContent=labels[k]||k;sel.append(o)});sel.value=(data.render_options||[])[0]||'native'}
function setVideo(){const s=document.getElementById('videoSlot'),sel=document.getElementById('videoSelect'),labels={predicted_full_gs:'预测位姿全程插值 · 全量GS',predicted:'预测位姿全程插值 · 筛选GS',ground_truth_full_gs:'真实位姿全程轨迹 · 全量GS',ground_truth:'真实位姿全程轨迹 · 筛选GS',wide:'120°轨道',wiggle:'小范围摆动',turntable:'转台'};s.replaceChildren();sel.replaceChildren();(data.video_options||[]).forEach(k=>{const o=document.createElement('option');o.value=k;o.textContent=labels[k]||k;sel.append(o)});if((data.video_options||[]).length){sel.value=data.video_options.includes('predicted_full_gs')?'predicted_full_gs':(data.video_options.includes('predicted')?'predicted':data.video_options[0]);const show=()=>{s.replaceChildren();const v=document.createElement('video');v.controls=true;v.loop=true;v.preload='metadata';v.src=`/media/${data.selected_id}/video/${sel.value}?v=${Date.now()}`;s.append(v)};sel.onchange=show;show()}else{const d=document.createElement('div');d.className='missing';d.textContent='没有渲染视频';s.append(d)}}
