...
def main():
    ...
    for query_exp in sorted(ROOT.glob("[0-9][0-9]_*/[0-9]*views/querysplat")):
        ...
        for method, gs_variant, method_dir, render_dir, ply_name in configs:
            ...
            if method == "ZipSplat":
                timing=json.loads((output/"run_stats.json").read_text()); pose=0.; recon=timing["inference_seconds"]; render_time=timing["render_seconds"]
            else:
                ...
                render_time=json.loads((output/render_dir/"render_manifest.json").read_text())["render_seconds"]
            per_experiment.append({"scene":scene,"views":views,"method":method,"gs_variant":gs_variant,"images":len(refs),"psnr":float(np.mean(psnrs)),"ssim":float(np.mean(ssims)),"lpips_alex":float(np.mean(lp)),"gaussians":ply_count(output/ply_name),"pose_depth_seconds":pose,"reconstruction_seconds":recon,"render_all_input_views_seconds":render_time})
    ...
    for (views,method,variant),rows in sorted(grouped.items()):
        aggregate.append({"views":views,"method":method,"gs_variant":variant,"experiments":len(rows),**{k:avg(rows,k) for k in ("psnr","ssim","lpips_alex","gaussians","pose_depth_seconds","reconstruction_seconds","render_all_input_views_seconds")}})
    overall=[]
    ...
    lines=["# 前60帧重建配置对比报告","","## 评测矩阵","","共10个场景、4/8/12/16视角，每种配置40组、400张输入视角图像。QuerySplat两种方法分别比较全量GS与opacity≥0.05筛选GS；ZipSplat的原生PLY是完整模型输出。","","## 总体结果","","| 方法 | GS策略 | PSNR ↑ | SSIM ↑ | LPIPS ↓ | 平均GS数 | 重建s | 全部输入视角渲染s |","|---|---|---:|---:|---:|---:|---:|---:|"]
    for r in overall: lines.append(f"| {r['method']} | {r['gs_variant']} | {r['psnr']:.3f} | {r['ssim']:.4f} | {r['lpips_alex']:.4f} | {r['gaussians']:.0f} | {r['reconstruction_seconds']:.3f} | {r['render_all_input_views_seconds']:.4f} |")
    lines += ["","## 分视角结果","","| 视角 | 方法 | GS策略 | PSNR ↑ | SSIM ↑ | LPIPS ↓ | GS数 | 重建s | 渲染s |","|---:|---|---|---:|---:|---:|---:|---:|---:|"]
    for r in aggregate: lines.append(f"| {r['views']} | {r['method']} | {r['gs_variant']} | {r['psnr']:.3f} | {r['ssim']:.4f} | {r['lpips_alex']:.4f} | {r['gaussians']:.0f} | {r['reconstruction_seconds']:.3f} | {r['render_all_input_views_seconds']:.4f} |")
    lines += ["","## 口径说明","","- 图片指标是输入视角重建指标；真实图和预测图统一中心裁剪/缩放至256×256。","- QuerySplat全量与筛选结果均从对应PLY通过同一渲染器重新渲染，以避免把内存渲染和PLY重载混为一谈。","- QuerySplat同一方法的全量/筛选配置共享同一次重建，因此重建时间相同；差异在GS数、渲染耗时与图像结果。","- ZipSplat使用共享的QuerySplat预测相机，位姿时间不计入ZipSplat重建时间。","- LPIPS使用AlexNet v0.1；PSNR/SSIM越高越好，LPIPS越低越好。",""]
